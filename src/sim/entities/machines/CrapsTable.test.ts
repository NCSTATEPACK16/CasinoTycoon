...
import { CRAPS_BALANCE, crapsExpectedRtp } from '../../../data/balance';
import { Rng } from '../../rng';
...
describe('CrapsTable', () => {
  it('pays out close to the configured RTP over 10k test rounds', () => {
    const table = new CrapsTable('obj-1');
    const rng = new Rng(1234);
    let paid = 0;
    const rounds = 10_000;
    for (let i = 0; i < rounds; i++) paid += table.testSpin(rng);
    const rtp = paid / (rounds * table.costToPlay);
    expect(rtp).toBeGreaterThan(crapsExpectedRtp() - 0.05);
    expect(rtp).toBeLessThan(crapsExpectedRtp() + 0.05);
  });
  ...
  it('wears down with real rounds and breaks at zero reliability', () => {
    ...
    const roundsToBreak = Math.ceil(100 / CRAPS_BALANCE.wearPerPlay);
  ...
  it('exposes a fast cadence tuned between slots and blackjack', () => {
    const table = new CrapsTable('obj-1');
    expect(table.costToPlay).toBe(CRAPS_BALANCE.costToPlay);
    expect(table.cadence.intervalTicks).toBe(CRAPS_BALANCE.playIntervalTicks);
    expect(table.cadence.playsMin).toBe(CRAPS_BALANCE.playsMin);
    expect(table.cadence.playsMax).toBe(CRAPS_BALANCE.playsMax);
  });
});
